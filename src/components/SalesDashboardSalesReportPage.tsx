...
export function SalesDashboardSalesReportPage({
  salesEntries
}: SalesDashboardSalesReportPageProps) {
  ...
  const bottleQty = filteredEntries.reduce(
    (sum, entry) => sum + (parseInt(entry.releasedBottles, 10) || 0),
    0
  );
  const blisterQty = filteredEntries.reduce(
    (sum, entry) => sum + (parseInt(entry.releasedBlister, 10) || 0),
    0
  );
      ...
      <div className="mb-4 flex items-center justify-between gap-4 no-print">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            value={reportDate}
            onChange={(event) =>
              setReportDate(event.target.value || new Date().toISOString().slice(0, 10))
            }
        ...
        <div className="flex items-center justify-end gap-2 no-print">
          ...
          <button type="button" onClick={handlePrint} className="rounded border border-black px-3 py-1">
            Print Report
          </button>
        </div>
      </div>
        ...
        <div className="border border-black p-3">
              ...
              <table className="w-full border border-black border-collapse">
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="border border-black px-2 py-1 text-right">{bottleQty}</td>
                    <td className="border border-black px-2 py-1 text-right">{formatMoney(getPrice("synbiotic (bottle)"))}</td>
                    <td className="border border-black px-2 py-1 text-right">
                      {formatMoney(bottleQty * getPrice("synbiotic (bottle)"))}
                    </td>
                  </tr>
                  <tr>
                    <td className="border border-black px-2 py-1">Synbiotic+ (Blister)</td>
                    <td className="border border-black px-2 py-1 text-right">{blisterQty}</td>
                    <td className="border border-black px-2 py-1 text-right">{formatMoney(getPrice("synbiotic (blister)"))}</td>
                    <td className="border border-black px-2 py-1 text-right">
                      {formatMoney(blisterQty * getPrice("synbiotic (blister)"))}
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            ...
            <table className="w-full border border-black border-collapse">
              <thead>
                <tr>
                  <th className="border border-black px-2 py-1 text-left">Upgrades</th>
          ...
          <div className="mt-4">
            <table className="w-full border border-black border-collapse">
              <thead>
                <tr>
                  <th className="border border-black px-2 py-1 text-left" colSpan={3}>
              ...
              <tbody>
                {(bankDetails.length ? bankDetails : [{ memberName: "-", reference: "-", amount: 0 }]).map(
                  (row, index) => (
                    <tr key={`bank-${index}`}>
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            <table className="w-full border border-black border-collapse">
              ...
              <tbody>
                {(mayaDetails.length ? mayaDetails : [{ memberName: "-", reference: "-", amount: 0 }]).map(
                  (row, index) => (
                    <tr key={`maya-${index}`}>
                      <td className="border border-black px-2 py-1">{row.memberName}</td>
                      <td className="border border-black px-2 py-1">{row.reference}</td>
            ...
            <table className="w-full border border-black border-collapse">
              ...
              <tbody>
                {(gcashDetails.length ? gcashDetails : [{ memberName: "-", reference: "-", amount: 0 }]).map(
                  (row, index) => (
                    <tr key={`gcash-${index}`}>
                      <td className="border border-black px-2 py-1">{row.memberName}</td>
                      <td className="border border-black px-2 py-1">{row.reference}</td>
                  ...
                  {filteredEntries.map((entry, index) => {
                    ...
                    return (
                      <tr key={entry.id}>
                        ...
                        <td className="border border-black px-2 py-1">
                          {part1
                            ? `${titleCase(part1.mode || "cash")} ${part1.reference ? `(${part1.reference})` : ""} - ${formatMoney(part1.amount)}`
                            : "-"}
                        </td>
                        <td className="border border-black px-2 py-1">
                          {part2
                            ? `${titleCase(part2.mode)} ${part2.reference ? `(${part2.reference})` : ""} - ${formatMoney(part2.amount)}`
                            : "-"}
                        </td>
